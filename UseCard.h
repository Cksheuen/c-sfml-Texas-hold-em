...
private:
  ...
  Font font, mid_font, normal_font;
  Text text, mid_text;
...
public:
  float x, y, angle;
  UseCard(sf::RenderWindow &windowSet, int numberSet, enum CardType cardTypeSet,
          float xSet, float ySet, float widthSet, float heightSet, Font fontSet,
          Font normal_fontSet)
      : window(windowSet), number(numberSet), cardType(cardTypeSet), x(xSet),
        y(ySet), width(widthSet), height(heightSet),
        Cardshape(sf::Vector2f(width, height)), signWidth(width / 4),
        font(fontSet), normal_font(normal_fontSet) {
    Cardshape.setPosition(x, y);
    shader = new UseShader<RectangleShape>(windowSet, clock, Cardshape,
                                           "shaders/card/card.vert",
                                           "shaders/card/card.frag");
    ...
    if (number >= 1 && number <= 10) {
      ...
      signSprite.setTextureRect(
          IntRect(startX, startY, endX - startX, endY - startY));
      signSprite.setScale(signWidth / signSprite.getLocalBounds().width,
                          signWidth / signSprite.getLocalBounds().height);

      for (int i = 0; i < number; i++) {
        if (number <= 3) {
          signSprite.setPosition(width / 2 - signWidth / 2,
                                 height / (number + 1) * (i + 1) -
                                     signWidth / 2);
        } else {
          ...
          if (inRow == 0)
            signSprite.setPosition(width / 3 * (i % 2 + 1) - signWidth / 2,
                                   height / (number / 2 + 1) * (i / 2 + 1) -
                                       signWidth / 2);
          else if (i + 1 > outRow && i + 1 <= outRow + inRow)
            signSprite.setPosition(width / 2 - signWidth / 2,
                                   height / (inRow + 1) * (i + 1 - outRow) -
                                       signWidth / 2);
          else
            signSprite.setPosition(
                width / 3 * (max(i - inRow, 0) / outRow + 1) - signWidth / 2,
                height / (outRow + 1) * min(i + 1, number - i) - signWidth / 2);
        }
  ...
  void disappearCard() {
    ...
    for (int i = 0; i < number; i++)
      cardTypeSign[i]->disappearShader();
  }

  void saveAsPng(Sprite sprite) {

    sf::RenderTexture renderTexture;
    renderTexture.create(sprite.getGlobalBounds().width,
  ...
  void setRotation(float angleSet) { angle = angleSet; }
  ...
  bool Hover() {
    ...
    if (originalBounds.contains(mousePos.x, mousePos.y)) {
      string cardTypeStr =
          cardTypeNumMap[cardType] + " " + number_to_string(number);
      hoverText.setString(cardTypeStr);

      hoverText.setPosition(x - cardTypeStr.length(), y - height / 2);
      hoverState = true;
      return true;
    } else {
      hoverState = false;
  ...
  void scale(float x, float y) { completeCard.setScale(x, y); }
};
